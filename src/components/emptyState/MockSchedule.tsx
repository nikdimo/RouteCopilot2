...
export const MockSchedule: React.FC<MockScheduleProps> = ({ animationState }) => {
    ...
    const m2Y = useSharedValue(CARD_HEIGHT + GAP);
    const m3Y = useSharedValue((CARD_HEIGHT + GAP) * 2);
    const m4Y = useSharedValue((CARD_HEIGHT + GAP) * 3);

    // Initialize physical Y position for the floating new meeting directly at Gap 1 so it doesn't "drop" from 0
    const newMY = useSharedValue(CARD_HEIGHT + GAP);
    ...
    useEffect(() => {
        ...
        const p1 = 0;
        const p2 = CARD_HEIGHT + GAP;
        const p3 = (CARD_HEIGHT + GAP) * 2;
        const p4 = (CARD_HEIGHT + GAP) * 3;
        const insertOffset = CARD_HEIGHT + GAP;
    ...
    const ExistingCard = ({ meet, style }: { meet: any, style: any }) => (
        <Animated.View style={[styles.card, styles.existingCard, style]}>
            <View style={styles.timeColumn}>
                <Text style={styles.timeText}>{meet.time}</Text>
                <Text style={styles.timeTextSub}>{meet.endTime}</Text>
            </View>
            <View style={styles.infoColumn}>
                <Text style={styles.title}>{meet.title}</Text>
                <Text style={styles.subtitle}>Existing Meeting</Text>
            </View>
        </Animated.View>
    );
    ...
    return (
        <View style={styles.container}>
            <View style={styles.header}>
                <Text style={styles.headerTitle}>Calculating Route Options</Text>
                <Text style={styles.headerSubtitle}>Evaluating gaps for optimum fit...</Text>
            </View>
            ...
            <View style={styles.listContainer}>
                ...
                <Animated.View style={[styles.card, styles.newCard, isLocked && styles.newCardLocked, newMStyle]}>
                    <View style={styles.timeColumn}>
                        <Text style={[styles.timeText, styles.newTimeText, isLocked && styles.newTimeLocked]}>{newMeetingTime}</Text>
                        <Text style={[styles.timeTextSub, styles.newTimeSub, isLocked && styles.newTimeLocked]}>{newMeetingEndTime}</Text>
                    </View>
                    <View style={styles.infoColumn}>
                        <View style={styles.badgeRow}>
                            {isLocked ? (
                                <View style={[styles.badge, styles.badgeSuccess]}>
                                    <Text style={styles.badgeTextSuccess}>BEST MATCH</Text>
                                </View>
                            ) : (
                                <View style={[styles.badge, styles.badgeEvaluating]}>
                                    <Text style={styles.badgeTextEvaluating}>EVALUATING</Text>
                                </View>
                            )}
                            <Text style={[styles.detourText, { color: isLocked ? '#15803d' : '#0369a1' }]}>
                                {detourText}
                            </Text>
                        </View>
                        <Text style={[styles.title, styles.newTitle, isLocked && styles.newTitleLocked]}>New Prospect</Text>
                    </View>
                    {isLocked && (
